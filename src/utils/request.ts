 ...
 * @type string
 */
export const BASE_URL: string = process.env.VUE_APP_API_URL || '/api/v2'

type HttpCode = {
  [key: number]: string
}

const codeMessage: HttpCode = {
  200: '服务器成功返回请求的数据。',
  201: '新建或修改数据成功。',
...
const errorHandler = (error: ResponseError) => {
  const errorText = (error.response && codeMessage[error.response.status]) || error.message

  notification.error({
    message: `请求错误 ${status}: ${error.request.url}`,
